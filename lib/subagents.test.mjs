...
const { getSubagentProfileEditorSource, isSubagentProfileOverridden } = await createJiti(import.meta.url).import("./subagent-profile-precedence.ts");
...
test("built-in editors reuse the effective writable override without resetting its fields", () => {
  const builtin = profile({ name: "explore", scope: "builtin", tools: ["read", "grep", "find", "ls"] });
  const global = profile({ name: "EXPLORE", scope: "global", model: "provider/global", systemPrompt: "Keep global instructions" });
  const workspace = profile({ name: "explore", scope: "workspace" });
  const project = profile({ name: "explore", scope: "project", model: "provider/project", tools: [] });
  const unrelated = profile({ name: "other", scope: "project" });

  assert.equal(getSubagentProfileEditorSource(builtin, [builtin, unrelated]), builtin);
  assert.equal(getSubagentProfileEditorSource(builtin, [builtin, global]), global);
  assert.equal(getSubagentProfileEditorSource(builtin, [project, global, builtin, workspace]), project);
  // A workspace source is still read-only; a new project override can supersede it.
  assert.equal(getSubagentProfileEditorSource(builtin, [builtin, global, workspace]), builtin);
  assert.equal(getSubagentProfileEditorSource(workspace, [builtin, project, workspace]), workspace);
  // Selecting a particular writable source must not silently switch its scope.
  assert.equal(getSubagentProfileEditorSource(global, [builtin, global, project]), global);
});

test("editing built-in settings creates removable same-name overrides without expanding tool permissions", async () => {
  for (const scope of ["global", "project"]) {
    for (const name of ["explore", "plan", "general-purpose"]) {
      const cwd = await mkdtemp(join(tmpdir(), "pi-web-builtin-edit-"));
      try {
        const builtin = listSubagentProfileSources(cwd).find((item) => item.scope === "builtin" && item.name === name);
        const original = structuredClone(builtin);
        const saved = saveSubagentProfile(cwd, scope, {
          ...builtin,
        ...
        const effective = resolveSubagentProfile(cwd, name);
        assert.equal(effective.scope, scope);
        assert.equal(effective.model, "provider/custom-model");
        assert.equal(effective.systemPrompt, "Custom instructions for this agent.");
        assert.equal(effective.thinking, "high");
        assert.deepEqual(effective.tools, original.tools);
        assert.equal(effective.loadSkills, original.loadSkills);
        assert.equal(effective.loadExtensions, original.loadExtensions);
        assert.deepEqual(listSubagentProfileSources(cwd).find((item) => item.scope === "builtin" && item.name === name), original);
        deleteSubagentProfile(cwd, scope, name);
        assert.deepEqual(resolveSubagentProfile(cwd, name), original);
      } finally {
        deleteSubagentProfile(cwd, scope, name);
