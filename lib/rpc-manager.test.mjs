...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(subagentSource, /withSubagentExtensionTools\(profile\.tools, remoteWorkspace[\s\S]*?extensionToolNames\.filter\([\s\S]*?: extensionToolNames\)/);
  assert.match(subagentSource, /remoteWorkspace \? \["bash"\] : settingsManager\.getDefaultTools\(\)/);
  ...
  assert.match(startupSource, /preferPiWebSubagentExtension\(preferRemoteWorkspaceExtension\(base, Boolean\(remoteWorkspace\)\)\)/);
});
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(startupSource, /const trustReloadOptions = remoteWorkspace\s*\? \{ resolveProjectTrust: async \(\) => false \}\s*: subagentResources[\s\S]*?subagentLoadsResources[\s\S]*?projectTrustReloadOptions\(sessionCwd, agentDir\)/);
