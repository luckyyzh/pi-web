...
// drawing a header-only pill with no body.
function hasVisibleContent(widget: ExtensionWidgetItem): boolean {
  return widget.lines.join("\n").trim() !== "";
}
...
export function ExtensionWidgets({ widgets }: { widgets: ExtensionWidgetItem[] }) {
  ...
  const visibleWidgets = widgets.filter(hasVisibleContent);
  if (visibleWidgets.length === 0) return null;

  const expandedWidget = visibleWidgets.find((widget) => (
