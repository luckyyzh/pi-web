...
export function AppShell() {
  ...
  const [activeTopPanel, setActiveTopPanel] = useState<"agents" | "branches" | "system" | "tools" | "session" | "persona" | null>(null);
  ...
  const toggleTopPanel = useCallback((
    panel: "agents" | "branches" | "system" | "tools" | "session" | "persona",
  ...
  const sidebarContent = (
    <>
      ...
      <div style={{ padding: "8px", flexShrink: 0, display: "flex", justifyContent: "space-between", gap: 4 }}>
        ...
        {(() => {
          const sshDisabled = !activeCwd && !selectedSession?.cwd && !newSessionCwd;
          return (
            <button
              type="button"
              ...
              style={{
                flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                height: 32, padding: 0, background: "none", border: "none",
                borderRadius: 9, color: "var(--text-muted)", cursor: sshDisabled ? "default" : "pointer",
                fontSize: 12, opacity: sshDisabled ? 0.35 : 1,
                transition: "background 0.12s, color 0.12s",
              }}
              onMouseEnter={(event) => { if (!sshDisabled) { event.currentTarget.style.background = "var(--bg-hover)"; event.currentTarget.style.color = "var(--text)"; } }}
              onMouseLeave={(event) => { event.currentTarget.style.background = "none"; event.currentTarget.style.color = "var(--text-muted)"; }}
            >
  ...
  const renderChatToolbarActions = (mobile: boolean) => {
    ...
    return (
      <div style={{ display: "flex", alignItems: "stretch", height: "100%" }}>
        ...
        <button
          type="button"
          onClick={() => toggleTopPanel("persona", mobile)}
          ...
          onMouseLeave={(event) => { event.currentTarget.style.color = activeTopPanel === "persona" ? "var(--text)" : "var(--text-muted)"; }}
          data-mobile-toolbar-action={mobile ? "persona" : undefined}
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: personaHasContent ? "var(--accent)" : "var(--text-dim)", flexShrink: 0 }} aria-hidden="true">
            <circle cx="12" cy="8" r="4" />
            <path d="M4 21c0-4 3.5-6 8-6s8 2 8 6" />
          </svg>
          {!mobile && <span>{translate("persona.label")}</span>}
        </button>
      </div>
    );
  };
      ...
      >
        {mobile ? (
          ...
          <>
            ...
            {tokens && tokens.input + tokens.cacheRead > 0 && (
              <span style={{ display: "flex", alignItems: "center", gap: 4 }} title={translate("stats.cacheHitRate")}>
                <svg width="12" height="12" viewBox="0 0 10 10" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                  <path d="M8.5 5a3.5 3.5 0 1 1-1-2.45" /><polyline points="6.5 1.5 8.5 2.5 7.5 4.5" />
                </svg>
                {((tokens.cacheRead / (tokens.input + tokens.cacheRead)) * 100).toFixed(1)}%
              </span>
            )}
            {costText && (
