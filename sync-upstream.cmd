...
REM  - Fetches upstream (agegr/pi-web), reports ahead/behind
REM  - Merges upstream/main if there is anything new
...
if errorlevel 1 (
  echo [ERROR] No "upstream" remote. Add it first:
  echo        git remote add upstream https://github.com/agegr/pi-web.git
  pause
  exit /b 1
)
...
REM ---- Count ahead/behind vs upstream/main ----
for /f "tokens=1,2" %%x in ('git rev-list --left-right --count main...upstream/main') do (
  set LOCAL=%%x
  set REMOTE=%%y
)

echo [2/3] main is %LOCAL% ahead, %REMOTE% behind upstream/main.

if "%REMOTE%"=="0" (
  echo [OK] main already contains everything from upstream/main. Nothing to merge.
  pause
  exit /b 0
)

echo [3/3] Merging upstream/main into main...
